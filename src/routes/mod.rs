...
/// Create stateless routes for the Integrations module.
///
...
///
/// ```ignore
/// let module = IntegrationsModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::IntegrationsModule) -> Router<()> {
    Router::new()
        .merge(create_integration_connector_routes(module.integration_connector_service.clone()))
        .merge(create_integration_event_routes(module.integration_event_service.clone()))
}

/// Read-only routes for the Integrations module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_integrations_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_integrations_routes(module: &crate::IntegrationsModule) -> Router<()> {
    Router::new()
        .merge(create_integration_connector_read_routes(module.integration_connector_service.clone()))
        .merge(create_integration_event_read_routes(module.integration_event_service.clone()))
}

/// Get all routes (stateless) for the Integrations module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::IntegrationsModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Integrations module.
///
...
pub fn create_combined_routes(module: &crate::IntegrationsModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Integrations module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::IntegrationsModule) -> Router<AppState> {
    Router::new()
