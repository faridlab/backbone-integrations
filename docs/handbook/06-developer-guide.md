...
metaphor migration generate Invoice integrations
metaphor migration run
...
(`integrations` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed integrations          # run Rust seeders
metaphor migration generate-seeds integrations  # emit SQL seed files
```
